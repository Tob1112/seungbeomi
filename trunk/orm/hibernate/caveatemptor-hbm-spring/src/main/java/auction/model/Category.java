 ...
 * <p>
 * Each Category can have many items (and an item can be in many categories).
 * This is a many-to-many relationship. There are four strategies how you can
 * map it.
 * <p>
 ...
 * only unidirectional navigation. The <tt>Item</tt> class does not know
 * anything about this collection or the components - no shared references.
 * <p>
 * Finally, the map <tt>itemsAndUser</tt> represents the many-to-many
 * association with a ternary relationship using a hash map. This map has item
 * objects as keys, and user objects as values. The underlying many-to-many join
 * table has three columns, <tt>CATEGORY_ID</tt>, <tt>ITEM_ID</tt>, and
 * <tt>ADDED_BY_USER_ID</tt>. This strategy allows you to map an additional
 * column (the user foreign key) of a many-to-many join table without writing an
 * intermediate entity or component class.
 * 
 * @see Item
 * @see CategorizedItem
...
public class Category implements Serializable, Comparable {
	 ...
	 * Full constructor
	 */
	public Category(String name, List<Category> childCategories,
			Category parentCategory, List<Item> items,
	...
	// Many-to-many with additional columns on join table, intermediate
	// component class
	...
	public boolean equals(Object o) {
		...
		if (!name.equals(category.name))
			return false;
		return !(parentCategory != null ? !parentCategory
				.equals(category.parentCategory)
	...
	public int hashCode() {
		...
		result = 29 * result
				+ (parentCategory != null ? parentCategory.hashCode() : 0);
