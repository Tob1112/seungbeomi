 ...
 * stores <tt>AuditLogRecord</tt> instances with the helper class
 * <tt>AuditLog</tt>. Because you can't access a <tt>Session</tt> inside its own
 * interceptor, a temporary second <tt>Session</tt> is opened on the same JDBC
 * connection and inside the same transaction, just to store the audit log
 * objects.
 * 
 * @see Auditable
 * @see AuditLog
