 ...
 * <p>
 * If the "successful" property is used in a legacy situation (see book chapter
 * 8), it is no longer an immutable class with consequences for second-level
 * caching.
 * 
 * @see Item
 * @see User
...
public class Bid implements Serializable, Comparable {
	...
	public boolean equals(Object o) {
		...
		if (!getItem().getId().equals(bid.getItem().getId()))
			return false;
		if (!(created.getTime() == bid.created.getTime()))
			return false;
	...
	public String toString() {
		return "Bid ('" + getId() + "'), " + "Created: '" + getCreated() + "' "
				+ "Amount: '" + getAmount() + "'";
	}
