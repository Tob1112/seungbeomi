 ...
 * <p>
 * This is really a very special mapping. The CategorizedItem class represents a
 * join table. The ER model for this is really a many-to-many association, but
 * instead of two entities and two collections, we mapped this as two
 * one-to-many associations between three entities. One of the motivation for
 * this are the additional attributes on the association table (not only two
 * FKs): username and creation date.
 * <p>
 * To create a new link between a Category and an Item, instantiate an object of
 * this class using the right constructor. Note that the Category and Item have
 * to have an identifier value, they have to be either in persistent or detached
 * state. Transient Category and Item instances don't work.
 * 
...
public class CategorizedItem implements Serializable, Comparable {
	...
	private Id id;
	private String username; // This could also be a many-to-one association to
								// User
	...
	public int compareTo(Object o) {
		...
		if (o instanceof CategorizedItem)
			return getDateAdded().compareTo(
					((CategorizedItem) o).getDateAdded());
		return 0;
	}

	public String toString() {
		return "Added by: '" + getUsername() + "', " + "On Date: '"
				+ getDateAdded();
	}
