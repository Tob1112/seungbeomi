...
public class Locking extends HibernateIntegrationTest {
	...
	public void nonRepeatableRead() {
		...
		// Would produce repeatable read if supported by DBMS, but also result
		// in dead lock
		...
		String newDescription = (String) s
				.createQuery(
						"select i.description from Item i where i.id = :itemid")
				.setParameter("itemid", 1l).uniqueResult();
