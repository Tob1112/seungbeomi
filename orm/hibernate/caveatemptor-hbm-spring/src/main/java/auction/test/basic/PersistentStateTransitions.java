...
import auction.dao.CategorizedItemDAO;
import auction.dao.DAOFactory;
import auction.dao.ItemDAO;
import auction.dao.ShipmentDAO;
...
public class PersistentStateTransitions extends HibernateIntegrationTest {
	...
	public void storeAndLoadItem() {
		...
		MonetaryAmount initialPrice = new MonetaryAmount(new BigDecimal(123),
				Currency.getInstance("USD"));
		MonetaryAmount reservePrice = new MonetaryAmount(new BigDecimal(333),
				Currency.getInstance("USD"));

		Item newItem = new Item("Testitem", "Test Description", user,
				initialPrice, reservePrice, startDate.getTime(),
				endDate.getTime());
	...
	public void manualCastOfBillingDetails() {
		...
		// An (instanceof CreditCard) check to user.defaultBillingDetails would
		// fail...
		// load() trick to retrieve the subclass instance, replacing the
		// superclass proxy
		CreditCard cc = (CreditCard) HibernateUtil
				.getSessionFactory()
				.getCurrentSession()
				.load(CreditCard.class, user.getDefaultBillingDetails().getId());
		assert cc.getType().equals(CreditCardType.MASTERCARD);

		// The previous operation produced a warning, because object identity
		// has now been violated,
		// one database row (well, several if it's a joined hierarchy mapping)
		// is now represented
	...
	public void createShipmentWithoutAuction() {
		...
		Shipment newShipment = new Shipment(user1.getShippingAddress(), user1,
				user2, 5);
		...
		Object[] result = (Object[]) s
				...
				.setParameter("shipId", newShipment.getId()).uniqueResult();
		s.close();
	...
	public void createShipmentForAuction() {
		...
		Object[] result = (Object[]) s
				...
				.addScalar("shipmentId", Hibernate.LONG).uniqueResult();
		s.close();
	...
	@Test(groups = "integration-hibernate", expectedExceptions = ObjectNotFoundException.class)
	...
	@Test(groups = "integration-hibernate", expectedExceptions = BusinessException.class)
	public void auctionNotActive() {
		...
		MonetaryAmount newAmount = new MonetaryAmount(bidAmount,
				Currency.getInstance("USD"));
		auction.placeBid(userDAO.findById(1l, false), newAmount, currentMaxBid,
				currentMinBid);
	...
	@Test(groups = "integration-hibernate", expectedExceptions = PermissionException.class)
	...
	public void placeBid() {
		...
		MonetaryAmount newAmount = new MonetaryAmount(bidAmount,
				Currency.getInstance("USD"));
		auction.placeBid(userDAO.findById(2l, false), newAmount, currentMaxBid,
				currentMinBid);
