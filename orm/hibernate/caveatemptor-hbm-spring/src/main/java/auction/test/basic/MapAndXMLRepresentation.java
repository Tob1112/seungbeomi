...
public class MapAndXMLRepresentation extends HibernateIntegrationTest {
	...
	public void workWithMaps() {
		...
		Map loadedItem = (Map) session.load("ItemEntity",
		...
		loadedItem = (Map) session.load("ItemEntity",
				(Serializable) ((Map) storedData.get("item1")).get("id"));
		...
		List queriedItems = session
				.createQuery("from ItemEntity where initialPrice >= :minPrice")
				.setParameter("minPrice", new BigDecimal(100)).list();
	...
	public void workWithXML() {
		...
		Element xmlItem = (Element) dom4jSession.get("ItemEntity",
				(Serializable) ((Map) storedData.get("item1")).get("id"));

		Element xmlUser = (Element) dom4jSession.get("UserEntity",
		...
		/*
		 * Pretty Print: try { OutputFormat format =
		 * OutputFormat.createPrettyPrint(); XMLWriter writer = new XMLWriter(
		 * System.out, format); writer.write( xmlItem); writer.write( xmlUser);
		 * } catch (IOException ex) { throw new RuntimeException(ex); }
		 * 
		 * <item id="2" seller-id="1"> <item-details initial-price="99"
		 * description="An item for auction"/> </item>
		 * 
		 * <user id="1" username="david"> <items-for-sale> <item id="2"
		 * seller-id="1"> <item-details initial-price="99"
		 * description="An item for auction"/> </item> <item id="3"
		 * seller-id="1"> <item-details initial-price="123"
		 * description="Another item for auction"/> </item> </items-for-sale>
		 * </user>
		 */
		...
		xmlItem = (Element) dom4jSession.get("ItemEntity",
				(Serializable) ((Map) storedData.get("item1")).get("id"));

		xmlItem.element("item-details").attribute("initial-price")
				.setValue("100");
		...
		assert xmlItem.element("item-details").attributeValue("initial-price")
				.equals("100");

		session.flush();
		session.clear();

		xmlUser = (Element) dom4jSession.get("UserEntity",
				(Serializable) ((Map) storedData.get("user")).get("id"));
		...
		Element loadedItem = (Element) dom4jSession.get("ItemEntity", new Long(
				newItem.attribute("id").getValue()));
