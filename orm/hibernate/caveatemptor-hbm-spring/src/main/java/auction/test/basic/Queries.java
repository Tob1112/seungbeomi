...
public class Queries extends HibernateIntegrationTest {
	...
	public void nativeSQLQueries() {
		...
		// Using aliases
		/*
		 * TODO: This doesn't work because of the <join many-to-one> in
		 * Item.hbm.xml result =
		 * session.createSQLQuery("select {i.*}, {ib.*} from ITEM i" +
		 * " join USERS u on i.SELLER_ID = u.USER_ID" +
		 * " left join ITEM_BUYER ib on ib.ITEM_ID = i.ITEM_ID " +
		 * " where u.USERNAME = :uname") .addEntity("i", Item.class)
		 * .addJoin("ib", "i.buyer") .setParameter("uname", "johndoe") .list();
		 * assert result.size() == 2; -->
		 * 
		 * /* TODO: This is tricky, don't know how to correctly fetch a joined
		 * component, HHH-2225 result =
		 * session.createSQLQuery("select {i.*}, {u.*}, {ba.*} from ITEM i" +
		 * " join USERS u on i.SELLER_ID = u.USER_ID" +
		 * " left join BILLING_ADDRESS ba on u.USER_ID = ba.USER_ID" +
		 * " where u.USERNAME = :uname") .addEntity("i", Item.class)
		 * .addJoin("u", "i.seller") .addJoin("ba", "u.billingAddress")
		 * .setParameter("uname", "johndoe") .list(); assert result.size() == 2;
		 */
		...
		result = session
				.createSQLQuery("select u.FIRSTNAME as fname from USERS u")
				.addScalar("fname").list();
		assert result.size() == 2;
		...
		result = session
				.createSQLQuery(
						"select c.RATING as rating from COMMENT c where c.FROM_USER_ID = :uid")
				.addScalar("rating",
						Hibernate.custom(StringEnumUserType.class, params))
				.setParameter("uid", 1l).list();
		 ...
		 * + " join USERS u on i.SELLER_ID = u.USER_ID" +
		 * " where u.USERNAME = :uname") .addEntity("i", Item.class)
		 * .addScalar("fname") .setParameter("uname", "johndoe") .list(); assert
		 * result.size() == 2; assert result.get(0) instanceof Object[];
		 */
	...
	public void basicCriteriaQueries() {
		...
		crit = session.createCriteria(User.class).add(
				Property.forName("email").eq("jd@mail.tld"));
		result = crit.list();
		assert result.size() == 1;

		// The same...
		crit = session.createCriteria(User.class).add(
				Restrictions.eq("email", "jd@mail.tld"));
		result = crit.list();
		assert result.size() == 1;

		// Restriction by component property with dot notation
		crit = session.createCriteria(User.class).add(
				Restrictions.eq("homeAddress.street", "Foostreet"));
		result = crit.list();
		assert result.size() == 2;
		...
		crit = session.createCriteria(User.class).add(
				Restrictions.like("firstname", "J", MatchMode.START));
		result = crit.list();
		assert result.size() == 1;

		// Find all auctions with at least 2 bids
		crit = session.createCriteria(Item.class).add(
				Restrictions.sizeGt("bids", 1));
		result = crit.list();
		assert result.size() == 1;
		...
		crit = session.createCriteria(User.class).add(
				Restrictions.or(Restrictions.and(
						Restrictions.like("firstname", "J%"),
						Restrictions.like("lastname", "D%")), Restrictions.in(
						"email", emails)));
		result = crit.list();
		assert result.size() == 1;
	...
	public void joinCriteriaQueries() {
		...
		result = crit.list();
		assert result.size() == 3; // Only 2 items in the database - join fetch
									// duplicates
		...
		crit = session.createCriteria(Item.class).createAlias("bids", "b")
	...
	public void projectionCriteriaQueries() {
		...
		crit = session.createCriteria(Item.class).setProjection(
				Projections.id());
		result = crit.list();
		assert result.size() == 2;
		assert result.get(0) instanceof java.lang.Long;

		// Return several attributes as an Object[] -- multicolumn projection
		// not supported?
		crit = session.createCriteria(Item.class).setProjection(
		...
		crit = session.createCriteria(Item.class).setProjection(
				Projections.projectionList().add(Property.forName("id"))
		...
		crit = session
				...
				.setProjection(
						Projections
								...
								.add(Projections.property("description").as(
										"itemDescription"))
								.add(Projections.property("initialPrice.value")
										.as("itemInitialPriceValue"))
								.add(Projections.property(
										"initialPrice.currency").as(
										"itemInitialPriceCurrency")))
				.setResultTransformer(
						new AliasToBeanResultTransformer(ItemPriceData.class));
		result = crit.list();
		assert result.size() == 2;
