 ...
 * @author Roy Clarkson
 */
public class EventSessionsMyFavoritesActivity extends EventSessionsListActivity {
	
	private static final String TAG = EventSessionsMyFavoritesActivity.class.getSimpleName();
	...
	private class DownloadSessionsTask extends AsyncTask<Void, Void, List<EventSession>> {
		...
		protected List<EventSession> doInBackground(Void... params) {
			try {
				...
				return getApplicationContext().getGreenhouseApi().sessionOperations().getFavoriteSessions(event.getId());
			} 
			catch(Exception e) {
