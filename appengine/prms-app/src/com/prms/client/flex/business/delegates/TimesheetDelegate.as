package com.prms.client.flex.business.delegates {
    ...
    public class TimesheetDelegate {
		 ...
		 * ２。DBからデータを読み込む
		 */
        public function fileUpload(vo:Timesheet):void {
        	var token:AsyncToken = _service.fileUpload(vo);
            token.addResponder(_responder);
        }
		 ...
		 * 勤務表アップロード保存
		 * */
        public function saveUploadTimesheet(vo:Timesheet):void {
        	var token:AsyncToken = _service.saveUploadTimesheet(vo);
            token.addResponder(_responder);
        }
    }
}
